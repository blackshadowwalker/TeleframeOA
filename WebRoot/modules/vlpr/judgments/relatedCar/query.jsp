...
<body>
		...
		<form action="${action}?&page.pageSize=${page.pageSize}" method="post" id="mainForm" >
			<table id="table-data-outter">
				...
				<div class="ConditionFiled">
					...
					<span class=titlelabel >时间阈值:</span> 
						<input type="text" name="timeThread"  value="${timeThread}" maxlength="3" />
						<span style="display:inline-block;width:15px;color:#000;margin-left:-20px;position:relative;">
							分
						</span>
		...
		<div id="list_div">
			<table id="table-view" cellpadding="0" cellspacing="1" style=""  >
				<tr class="row-header">
					...
					<td ondblclick="return sortQuery('resultCar.deviceId')" class="title-header" style="width:230px;" > 设备 </td>  
				...
				<s:iterator id="l" value="list" status="index">
					...
					<tr class="  <s:property value='var_class' />"
						...
						onclick="getReatedCar(<s:property value="#l.id" />)"
						>
	...
	<center>
		<span style="font-size:15px;font-weight: bold" >关联车辆信息</span>
		<span>(共&nbsp; <span id="releated-totalrow"> 0 </span> &nbsp;条记录)</span>
	</center>
	<div id="releated-view">  
		<div id="list_div">
			<table id="releated-table-view" cellpadding="0" cellspacing="1" style="width:100%;"  >
				<tr class="row-header">
					<td width="20"  class="hidden">ID</td>
	...
	<hr/>
		<script>
			...
			function getReatedCar(id){
				var timeThread =  $("input[name='timeThread']").val();
				var currentPage = ${page.totalPage};
				...
				$.ajax({
					...
					url: '${action}?method=querySub',
					dateType: 'json',
					success: function(data){
						console.log(data);
						var json = eval(data);
						$("#releated-table-view tr:not(:first)").remove();
						$("#releated-totalrow").html(json.length);
						for(var i=0; i<json.length; i++) {
						  	...
						  	html += "<td align=center class=hidden >"+json[i].id+"</td>";
						  	html += "<td align=center >"+json[i].plate+"</td>";
						  	html += "<td align=center >"+json[i].plateType+"</td>";
						  	html += "<td align=center >"+json[i].plateColor+"</td>";
						  	html += "<td align=center >"+json[i].carColor+"</td>";
						  	html += "<td align=center >"+json[i].carLogo+"</td>";
						  	html += "<td align=center >"+formatJsonDateTime(json[i].carTime)+"</td>";
						  	html += "<td align=center >"+json[i].devicePath+"</td>";
						  	html += "<td align=center >";
							html += "  <a class=operate href=\"${action}?method=view&id="+json[i].id+"\">";
							html += "	<img src=\"images/button/doc.gif\" title=\"详情\" style=\"cursor:pointer\"></img>";
							html += " </a>";
							html += "</td>";
						  	html +="</tr>";
							$("#releated-table-view").append(html);
						}
					},
