...
<script  >	
	function cancle(){
		location.href="<%= basePath%>/TaskHistoryAction?method=query";
	}
</script>
</head>
<body>
   ...
   //             checkRecursive: true,                
	...
	<div>
		...
		<form action="<%= basePath%>/TaskHistoryAction?method=save&id=${taskHistory.id}"   method="post" >
				...
				<tr>
					<td>
						<table id="table-data-inner" cellspacing="1">
							...
							<tr >
								<td width="10%" class="field-title">任务标识:</td>
								<td >
							...
							<tr >
								...
								<td >
									<input type="text" name="taskHistory.userId" value="${taskHistory.userId}" /> 	
									</td>
							</tr>
							<tr >
								<td width="10%" class="field-title">前端设备:</td>
								<td >
									<input type="text" name="taskHistory.deviceId" value="${taskHistory.deviceId}" /> 	
									</td>
							</tr>
							<tr >
								<td width="10%" class="field-title">状态:</td>
								<td >
											<select name="taskHistory.status" >
											<option value="1" selected>启用</option>
											<option value="-1">禁用</option>
										</select>
										<script>
											$("select[name='taskHistory.status']").val("${taskHistory.status }");
										</script>
									</td>
							</tr>
